...
// //   });
// const useGeneResultsData = (
//   params: { cancerSites: string[]; cancerTypes: string[]; genes: string[]; analysisType: string },
//   filterState: FilterState
// ) => {
//   // Move refs inside the hook
//   const geneMapRef = React.useRef<Record<string, string> | null>(null);          // symbol → ensembl
//   const reverseGeneMapRef = React.useRef<Record<string, string> | null>(null);   // ensembl → symbol

//   // Load map once
//   useEffect(() => {
//     if (geneMapRef.current) return;

//     const load = async () => {
//       try {
//         const r = await fetch("/api/gene_IDs");
//         if (!r.ok) throw new Error("Failed to fetch gene map");
//         const { genes } = await r.json();

//         const s2e: Record<string, string> = {};
//         const e2s: Record<string, string> = {};

//         Object.entries(genes as Record<string, string[]>).forEach(([symbol, list]) => {
...
//         geneMapRef.current = s2e;
//         reverseGeneMapRef.current = e2s;
//       } catch (e) {
//         console.error(e);
//         geneMapRef.current = {};
//         reverseGeneMapRef.current = {};
//       }
//     };
...
//   const [state, setState] = React.useState<GeneResultsState>({
//     resultsData: [], rawResultsData: [], cachedApiData: null, isLoading: false, error: null,
//     totalTumorSamples: 0, totalNormalSamples: 0, siteSampleCounts: [], availableSites: [], availableGenes: [], debugMessages: []
//   });
