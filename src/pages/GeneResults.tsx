...
//       error: filteredResults.length === 0 && filterState.selectedSites.length > 0 ? "No data matches the selected filters." : null,
//     }));
//   }, [filterState.selectedSites, filterState.selectedGenes, filterState.normalizationMethod, filterState.dataFormat, state.rawResultsData]);
...
//                   existing.gene_symbol === d.gene_symbol &&
//                   existing.normalizationMethod === d.normalizationMethod &&
//                   existing.dataFormat === d.dataFormat
//               )
//           ),
//         ],
//         resultsData: [
...
//         isLoading: false,
//       }));
//       if (cachedResults.some((d) => d.normalizationMethod === filterState.normalizationMethod && d.dataFormat === filterState.dataFormat)) {
//         return;
//       }
//     }
...
//           for (const format of ['raw', 'log2']) {
//             for (const norm of normalizationMethods) {
//               const geneData = apiData[format]?.[norm]?.[ensembl_id]?.[apiSiteKey] || {};
//               const data: any = {};
//               let warning: string | undefined = undefined;
//               let logfcMessage: string | undefined = undefined;
//               const tumorValues: number[] = [];
//               const normalValues: number[] = [];
...
//               if (sampleCountsBySite[site]?.normal <= 1 && norm === filterState.normalizationMethod && format === filterState.dataFormat) {
//                 logfcMessage = "Not enough normal samples for Differential Noise analysis.";
//               }

//               if (apiData.warning && norm === filterState.normalizationMethod && format === filterState.dataFormat) {
//                 warning = apiData.warning;
//               }
...
//                 );
//                 if (!upsertResult.success) {
//                   console.error(`Failed to upsert data for ${norm} (${format}):`, upsertResult.error);
//                 }
//               } catch (upsertError) {
//                 console.error(`Upsert failed for ${norm} (${format}):`, upsertError);
//               }
...
//                 });
//               } catch (cacheError) {
//                 console.warn(`Failed to cache data for ${norm} (${format}):`, cacheError);
//               }
//             }
//           }
//         }
...
//       setState((prev) => ({ ...prev, error: error.message || "An error occurred while fetching data.", isLoading: false }));
//     }
//   }, [params, getCachedData, setCachedData, generateCacheKey, filterState.selectedNoiseMetrics, filterState.selectedGroups, filterState.normalizationMethod, filterState.dataFormat]);
...
//           cancerTypes: params.cancerTypes,
//           normalizationMethod: norm,
//           dataFormat: format as 'raw' | 'log2', // Explicit type annotation
//           selectedNoiseMetrics: filterState.selectedNoiseMetrics,
//         })
//       )
//     );
...
//       if (cachedData) {
//         const norm = normalizationMethods[Math.floor(index / 2)];
//         const format: 'raw' | 'log2' = index % 2 === 0 ? 'raw' : 'log2'; // Explicit type
//         cachedResults.push(
//           ...cachedData.resultsData.map((d) => ({
//             ...d,
//             normalizationMethod: norm,
//             dataFormat: format,
//           } as GeneStats)) // Type assertion to ensure GeneStats compatibility
//         );
//       }
//     });
...
//   const downloadData = useCallback((format: "csv") => {
//     const data = resultsData.filter((d) => d.normalizationMethod === filterState.normalizationMethod && d.dataFormat === filterState.dataFormat);
//     let content = "";
//     let filename = `gene_analysis_${filterState.normalizationMethod}_${filterState.dataFormat}_${Date.now()}.csv`;

//     if (format === "csv") {
//       const excludedKeys = ["tumorValues", "normalValues", "normalizationMethod", "dataFormat", "logfcMessage"];
//       const keys = ["site", "gene_symbol", "ensembl_id", "cv_tumor", "mean_tumor", "std_tumor", "mad_tumor", "cv_squared_tumor", "cv_normal", "mean_normal", "std_normal", "mad_normal", "cv_squared_normal", "logfc", "tumorSamples", "normalSamples", "warning"];
//       const headers = keys.join(",");
//       const rows = data.map((row) =>
//         keys.map((key) => {
//           const value = row[key as keyof GeneStats];
...
//     a.click();
//     URL.revokeObjectURL(url);
//   }, [resultsData, filterState.normalizationMethod, filterState.dataFormat]);
...
//                 <div className="text-center text-red-600">Please select at least one gene or one site.</div>
//               ) : resultsData.length === 0 ? (
//                 <div className="text-center text-red-600">No data available for selected sites, genes, and data format.</div>
//               ) : (
//                 <>
//                   <Link
//                     to="/gene-analysis"
...
//                             <th className="text-left py-3 px-4 text-blue-700 font-semibold w-1/3">Normalization</th>
//                             <td className="py-3 px-4 text-blue-700">
//                               {filterState.dataFormat === 'raw' ? filterState.normalizationMethod.toUpperCase() : `log2(${filterState.normalizationMethod.toUpperCase()} + 1)`}
//                             </td>
//                           </tr>
//                           <tr className="border-b">
//                             <th className="text-left py-3 px-4 text-blue-700 font-semibold w-1/3">Data Format</th>
//                             <td className="py-3 px-4 text-blue-700">
//                               {filterState.dataFormat === 'raw' ? 'Raw' : 'Log2 Transformed'}
//                             </td>
//                           </tr>
//                           <tr className="border-b">
//                             <th className="text-left py-3 px-4 text-blue-700 font-semibold w-1/3">Genes</th>
...
//                       genes={params.genes}
//                       selectedSites={filterState.selectedSites}
//                       dataFormat={filterState.dataFormat}
//                       setDataFormat={(format) => dispatch({ type: "SET_DATA_FORMAT", payload: format })}
//                     />
//                     {params.analysisType === "cancer-specific" && (
//                       <AnalysisPlots
//                         isOpen={filterState.isAnalysisPlotsOpen}
...
// Define interfaces (already provided and aligned with GeneStats.ts)
interface FilterState {
  selectedGroups: string[];
  selectedGenes: string[];
  selectedSites: string[];
...
const useGeneResultsData = (params: {
  ...
  useEffect(() => {
    const filteredResults = state.rawResultsData.filter(
      (d) =>
        ...
        filterState.selectedGenes.includes(d.gene_symbol) &&
        d.normalizationMethod === filterState.normalizationMethod
        // Removed dataFormat filter to include both raw and log2 data
    );
  ...
  const fetchData = useCallback(async (sitesToFetch: string[], normalizationMethods: string[] = ["tpm", "fpkm", "fpkm_uq"]) => {
    ...
    cacheKeys.forEach((cacheKey, index) => {
      ...
      if (cachedData) {
        const norm = normalizationMethods[Math.floor(index / 2)];
        const format: 'raw' | 'log2' = index % 2 === 0 ? 'raw' : 'log2';
        cachedResults.push(
          ...cachedData.resultsData.map((d) => ({
            ...d,
            normalizationMethod: norm,
          ...
          for (const format of ['raw', 'log2'] as const) {
            for (const norm of normalizationMethods) {
              ...
              const noiseMetrics = {
                CV: "cv",
                Mean: "mean",
                "Standard Deviation with Mean": "std",
                MAD: "mad",
                "CV²": "cv_squared",
                "Differential Noise": "logfc",
              };
              ...
              if (sampleCountsBySite[site]?.normal <= 1 && norm === filterState.normalizationMethod) {
                logfcMessage = "Not enough normal samples for Differential Noise analysis.";
              }
  ...
  }, [params, getCachedData, setCachedData, generateCacheKey, filterState.selectedNoiseMetrics, filterState.selectedGroups, filterState.normalizationMethod]);
...
const GeneResults: React.FC = () => {
  ...
  const downloadData = useCallback((format: "csv") => {
    ...
    if (format === "csv") {
      const excludedKeys = ["tumorValues", "normalValues", "normalizationMethod", "logfcMessage"];
      const keys = ["site", "gene_symbol", "ensembl_id", "dataFormat", "cv_tumor", "mean_tumor", "std_tumor", "mad_tumor", "cv_squared_tumor", "cv_normal", "mean_normal", "std_normal", "mad_normal", "cv_squared_normal", "logfc", "tumorSamples", "normalSamples", "warning"];
      const headers = keys.join(",");
      const rows = data.map((row) =>
        keys.map((key) => {
          const value = row[key as keyof GeneStats];
          ...
          <div className="grid grid-cols-[320px_1fr] gap-6">
            ...
            <div className="flex-1">
              {isLoading ? (
                ...
                <div className="text-center text-red-600">No data available for selected sites, genes, and normalization method.</div>
              ) : (
                <>
                  <Link
                    to="/gene-analysis"
