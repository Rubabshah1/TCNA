                  ...
                  {currentResults.enrichment.length === 0 ? (
                    ...
                    <>
                      ...
                      {filterState.analysisMode === "enrichment" && filterState.selectedPathway && (
                        <>
                          <CollapsibleCard
                            title="Mean Expression for Pathway Genes"
                            className="mb-4"
                            downloadButton={
                          ...
                          <CollapsibleCard
                            title="Gene Noise Analytics (CV) for Pathway Genes"
                            className="mb-4"
                            downloadButton={
                          ...
                          <CollapsibleCard
                            title="Differential Noise - Log₂FC for Pathway Genes"
                            className="mb-4"
                            downloadButton={
                              <Button onClick={() => downloadData("noise_metrics")} variant="outline" size="sm" className="h-6 px-2 text-xs">
                                <Download className="h-4 w-4 mr-2" /> Download CSV
                          ...
                          <CollapsibleCard title="Mean Expression Heatmap for Pathway Genes" className="mb-4">
                            <div className="flex items-center space-x-2 mb-2 pl-6">
                              <Switch
                                id="mean-log2-switch"
                                checked={filterState.dataFormats.mean === "log2"}
                            ...
                            <PlotlyHeatmap
                              title="Mean Expression (Z-scores) for Pathway Genes"
                              data={Object.entries(resultsData[filterState.dataFormats.mean][filterState.normalizationMethod]?.gene_stats || {})
                                .filter(([gene]) => filterState.selectedPathway?.MatchingGenes.includes(gene))
                                .map(([, stats]) => stats)}
                          ...
                          <CollapsibleCard title="Gene Noise Heatmap for Pathway Genes" className="mb-4">
                            <div className="flex items-center space-x-2 mb-2 pl-6">
                              <Switch
                                id="cv-log2-switch"
                                checked={filterState.dataFormats.cv === "log2"}
                            ...
                            <PlotlyHeatmap
                              title="Coefficient of Variation (Z-scores) for Pathway Genes"
                              data={Object.entries(resultsData[filterState.dataFormats.cv][filterState.normalizationMethod]?.gene_stats || {})
                                .filter(([gene]) => filterState.selectedPathway?.MatchingGenes.includes(gene))
                                .map(([, stats]) => stats)}
                              xValues={filterState.selectedSites.flatMap((site) => [`${site} Normal`, `${site} Tumor`])}
                          ...
                          <CollapsibleCard title="Differential Noise Heatmap - Log₂FC for Pathway Genes">
                            <PlotlyHeatmap
...
// import PlotlyNetwork  from "@/components/charts/network"; // New import for network visualization
...
//   const fetchData = useCallback(
//     async (sitesToFetch: string[], normalizationMethods: string[] = ["tpm", "fpkm", "fpkm_uq"]) => {
//       if (!sitesToFetch.length || (!filterState.selectedGenes.length && filterState.analysisMode === "neighbors")) {
//         setState((prev) => ({
//           ...prev,
//           error: "Please select at least one cancer site and, for enrichment analysis, at least one gene.",
//           isLoading: false,
...
//       .map((g) => g.trim().toUpperCase())
//       .filter((g) => g.length > 0);
//     updateFilters({ selectedGenes: genes, geneInput: genes.join(", ") });
//   }, [filterState.geneInput, updateFilters]);
...
//   //       defaultOpen: false,
//   //     },
//   //     ...(filterState.analysisMode === "neighbors"
//   //       ? [{
//   //           title: "Genes",
//   //           id: "genes",
//   //           type: "checkbox",
...
//   //       : []),
//   //   ];
//   // }, [availableSites, resultsData, params.genes, filterState.analysisMode]);
// const customFilters: FilterSection[] = useMemo(() => {
//     const availableGenes = [
//       ...new Set(
//         ["raw", "log2"].flatMap((scale) =>
...
//       let filename = `pathway_analysis_${selectedSites.join("_")}_${filterState.dataFormats.mean}_${filterState.normalizationMethod}`;
//       if (type === "enrichment") {
//       const filename = `enrichment_results_${filterState.normalizationMethod}.csv`;
//       const headers = ["Rank", "Pathway/GO Term", "Database", "FDR", "Matching Genes"];
//       const rows = enrichedPathways.map((row) =>
//         [
//           row.Rank,
//           row.Term,
...
//       }
//     },
//     [enrichedPathways, filterState.normalizationMethod, resultsData, filterState.dataFormats, filterState.normalizationMethod, filterState.selectedSites, filterState.selectedGenes]
//   );
...
//               ) : error ? (
//                 <div className="text-center text-red-600">{error}</div>
//               ) : filterState.selectedSites.length === 0 || (filterState.selectedGenes.length === 0 && filterState.analysisMode === "neighbors") ? (
//                 <div className="text-center text-red-600">
//                   Please select at least one site and, for enrichment analysis, at least one gene.
//                 </div>
//               ) : currentResults.enrichment.length === 0 && currentResults.warning ? (
...
//                         </div>
//                       </div>
//                       {filterState.analysisMode === "enrichment" && currentResults.network?.length > 0 && (
//                         <CollapsibleCard
//                           title="Gene Interaction Network"
//                           className="mb-4"
//                           extra={
//                             <Switch
//                               checked={filterState.showNetwork}
//                               onCheckedChange={() => dispatch({ type: "TOGGLE_NETWORK" })}
//                               className="data-[state=checked]:bg-blue-600 data-[state=unchecked]:bg-gray-300"
//                             />
//                           }
//                         >
//                           {filterState.showNetwork && (
//                             <PlotlyNetwork
//                               nodes={filterState.selectedGenes}
//                               edges={currentResults.network}
//                               title="Gene Interaction Network (STRINGdb)"
//                             />
//                           )}
//                         </CollapsibleCard>
//                       )}
...
//                               />
//                             </CollapsibleCard>
//                             <CollapsibleCard title="Gene Noise" className="mb-4">
//                               <div className="flex items-center space-x-2 mb-2 pl-6">
//                                 <Switch
//                                   id="cv-log2-switch"
//                                   checked={filterState.dataFormats.cv === "log2"}
...
//                               />
//                             </CollapsibleCard>
//                             <CollapsibleCard title="Differential Noise Analysis (Log₂FC)">
//                               <PlotlyHeatmap
...
//     sample_counts: { [site: string]: { tumor: number; normal: number } };
//     available_sites: { id: number; name: string }[];
//   }>(); // ✅ Extended cache type to include sample counts
  
//   const filterTimeoutRef = useRef<NodeJS.Timeout | null>(null);
//   const normalizationMethods = ["tpm", "fpkm", "fpkm_uq"];

//   // ✅ CORRECT MERGE FUNCTION - Updated to handle sample counts
//   const mergeResultsData = useCallback((prev: ResultsData, newData: ResultsData): ResultsData => {
//     const merged: ResultsData = { raw: { ...prev.raw }, log2: { ...prev.log2 } };
//     normalizationMethods.forEach((norm) => {
//       ["raw", "log2"].forEach((format) => {
...
//           // MERGE GENE STATS: Combine per-site data
//           gene_stats: {
//             ...existing.gene_stats,
//             ...Object.keys(newDataSection.gene_stats).reduce((acc, gene) => {
//               if (!existing.gene_stats?.[gene]) {
...
//           // ✅ MERGE HEATMAP & NOISE: Simple merge
//           heatmap_data: {
//             ...existing.heatmap_data,
//             ...newDataSection.heatmap_data,
//           },
...
//   // ✅ NEW: Merge sample counts function
//   const mergeSampleCounts = useCallback((
//     prevCounts: { [site: string]: { tumor: number; normal: number } },
//     newCounts: { [site: string]: { tumor: number; normal: number } }
//   ) => {
...
//   const fetchData = useCallback(
//     async (sitesToFetch: string[], normalizationMethods: string[] = ["tpm", "fpkm", "fpkm_uq"]) => {
//       if (!sitesToFetch.length || (!filterState.selectedGenes.length)) {
//         setState((prev) => ({
//           ...prev,
...
//       // ✅ FALLBACK: Check individual site caches
//       const resultsBySite: { [site: string]: ResultsData } = {};
//       const sampleCountsBySite: { [site: string]: { tumor: number; normal: number } } = {};
//       const availableSitesBySite: { [site: string]: { id: number; name: string }[] } = {};
//       const sitesToFetchFromAPI: string[] = [];
...
//       // ✅ MERGE CACHED INDIVIDUAL SITES (including sample counts)
//       if (Object.keys(resultsBySite).length > 0) {
//         setState((prev) => {
//           let mergedData = prev.resultsData;
//           let mergedSampleCounts: { [site: string]: { tumor: number; normal: number } } = {};
...
//           // Merge sample counts from individual caches
//           Object.entries(sampleCountsBySite).forEach(([site, counts]) => {
//             mergedSampleCounts = mergeSampleCounts(mergedSampleCounts, { [site]: counts });
//           });

//           // Merge available sites
//           const allAvailableSites = Object.values(availableSitesBySite).flat();
//           if (allAvailableSites.length > 0) {
//             mergedAvailableSites = allAvailableSites;
//           }
...
//       setState((prev) => ({ ...prev, isLoading: true, error: null }));
//       try {
//         // ✅ BATCH FETCH ALL SITES AT ONCE
//         const cancerParam = sitesToFetchFromAPI.join(",");
//         const queryParams = new URLSearchParams({
//           cancer: cancerParam,
//           top_n: params.topN.toString(),
//           library: params.library || "KEGG_2021_Human",
//           ...(filterState.selectedGenes.length > 0 && {
//             genes: filterState.selectedGenes.join(","),
//           }),
//         });
...
//         // ✅ CACHE BATCH RESULT (includes sample counts & available sites)
//         setCachedData(batchCacheKey, apiData);
...
//   useEffect(() => {
//     const newSites = filterState.selectedSites.filter((site) => !state.fetchedSites.includes(site));
//     if (newSites.length > 0 && (filterState.selectedGenes.length > 0)) {
//       debouncedFetchData(newSites, normalizationMethods);
//     }
//   }, [filterState.selectedSites, params.cancerTypes, debouncedFetchData, filterState.selectedGenes]);
...
//       sites: searchParams.get("sites")?.split(",").filter(Boolean) || [],
//       cancerTypes: searchParams.get("cancerTypes")?.split(",").filter(Boolean) || [],
//       library: searchParams.get("library") || "KEGG_2021_Human", // ✅ ADD THIS LINE
//       genes: searchParams.get("genes")?.split(",").filter(Boolean) || [],
...
//               ) : error ? (
//                 <div className="text-center text-red-600">{error}</div>
//               ) : filterState.selectedSites.length === 0 || (filterState.selectedGenes.length === 0) ? (
//                 <div className="text-center text-red-600">
//                   Please select at least one site and one gene.
//                 </div>
//               ) : currentResults.enrichment.length === 0 && currentResults.warning ? (
//                 <Card className="shadow-lg p-6 text-center text-yellow-700 bg-yellow-50">
//                   <p className="text-lg">Warning: {currentResults.warning}</p>
