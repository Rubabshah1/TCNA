...
const useTumorResultsData = (
  ...
  const fetchData = useCallback(
    async ({
      ...
      if (!cancerSite || !selectedNoiseMetrics.length || state.isLoading || !normalizationMethods.includes(normalizationMethod)) {
      ...
      try {
        // Check cache for the selected normalization method
        const cacheKey = generateCacheKey({ cancerSite, cancerTypes, normalizationMethod });
        const cachedData = getCachedData(cacheKey);

        if (cachedData && cachedData.length > 0) {
          console.log(`Cache hit for ${normalizationMethod}:`, cacheKey, "Data length:", cachedData.length);
          setState((prev) => ({
            ...prev,
            rawResultsData: { ...prev.rawResultsData, [normalizationMethod]: cachedData },
        ...
        // Fetch data from API for selected normalization method and metrics
        const dataByNormalization: { [sampleId: string]: TumorData } = {};

        const promises: Promise<{ metric: string; data: any }>[] = [];
        for (const metric of metrics) {
          console.log(`Fetching API for metric=${metric}, norm=${normalizationMethod}, samples=${sampleBarcodes.length}`);
          promises.push(
            fetch(`/api/TH-metrics?cancer=${cancerSite}&method=${normalizationMethod}&metric=${metric}`)
              ...
              .then((data) => ({
                metric,
                data: data.log2[normalizationMethod]?.filter((item: any) =>
        ...
        for (const { metric, data } of metricResults) {
          if (data && data.length > 0) {
            for (const item of data) {
              ...
              if (!dataByNormalization[mapKey]) {
                dataByNormalization[mapKey] = {
                  sample: sampleInfo.barcode,
                  cancer_type: sampleInfo.cancerType || "Unknown",
                };
              }
              const fieldName = noiseMetrics[metric as keyof typeof noiseMetrics] || metric.toLowerCase();
              const value = item[normalizationMethod] != null ? item[normalizationMethod] : null;
              dataByNormalization[mapKey][fieldName] = value;
            }
          }
        }

        // Process and cache data for the selected normalization method
        const processedData = Object.values(dataByNormalization).filter((sampleData) =>
        ...
        console.log(`Caching data for ${normalizationMethod}:`, cacheKey, "Data length:", processedData.length);
        setCachedData(cacheKey, processedData);

        setState((prev) => ({
          ...prev,
          rawResultsData: { ...prev.rawResultsData, [normalizationMethod]: processedData },
  ...
  const debouncedFetchData = useCallback(
    (params: { cancerSite: string; cancerTypes: string[]; selectedNoiseMetrics: string[]; normalizationMethod: string }) => {
  ...
  }, [filterState.selectedNoiseMetrics, state.rawResultsData, filterState.normalizationMethod]);

  // Fetch data when cancerSite, cancerTypes, or normalizationMethod changes
  useEffect(() => {
    ...
    if (cachedData && cachedData.length > 0) {
      ...
      setState((prev) => ({
        ...prev,
        rawResultsData: { ...prev.rawResultsData, [filterState.normalizationMethod]: cachedData },
        resultsData: cachedData.filter((sampleData) =>
          filterState.selectedNoiseMetrics.some((metric) => {
            const fieldName = noiseMetrics[metric as keyof typeof noiseMetrics] || metric.toLowerCase();
            const value = sampleData[fieldName];
      ...
      console.log("Cache miss, triggering fetch for:", cacheKey);
      debouncedFetchData({
        cancerSite: params.cancerSite,
        cancerTypes: params.cancerTypes,
  ...
  }, [params.cancerSite, params.cancerTypes, filterState.normalizationMethod, getCachedData, generateCacheKey, debouncedFetchData, state.isLoading]);

  return { ...state, fetchData: debouncedFetchData };
};

// Remainder of the TumourResults component remains unchanged
...
//         // Fetch data from API for all metrics and normalizations
...
//         for (const normMethod of normalizationMethods) {
//           for (const metric of metrics) {
//             console.log(`Fetching API for metric=${metric}, norm=${normMethod}, samples=${sampleBarcodes.length}`);
//             promises.push(
